...
/**
 *  未读数改变了。通知服务器同步 installation 的badge
 */
static NSString *const kChatNotificationUnreadsUpdated = @"ChatUnreadsUpdated";

/**
 *  消息来了，通知聊天页面和最近对话页面刷新
 */
static NSString *const kChatNotificationMessageReceived = @"ChatMessageReceived";

/**
 *  消息到达对方了，通知聊天页面更改消息状态
 */
static NSString *const kChatNotificationMessageDelivered = @"ChatMessageDelivered";

/**
 *  对话的元数据变化了，通知页面刷新
 */
static NSString *const kChatNotificationConversationUpdated = @"ConversationUpdated";

/**
 *  聊天服务器连接状态更改了，通知最近对话和聊天页面是否显示红色警告条
 */
static NSString *const kChatNotificationConnectivityUpdated = @"ChatConnectStatus";

typedef void (^RecentConversationsCallback)(NSArray *conversations, NSInteger totalUnreadCount,  NSError *error);

@protocol ChatUserDelegate <NSObject>
 ...
 *  设置用户信息的 delegate
 */
@property (nonatomic, strong) id <ChatUserDelegate> userDelegate;
/**
...
/**
 *  在 AppDelegate 中的 application:didRemoteNotification 调用，来记录推送时的 convid，这样点击弹框打开后进入相应的对话
 *  @param userInfo
